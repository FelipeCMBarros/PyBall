...
pygame.display.set_caption("PyBall - Modo Fácil")
...
def desenhar_resultado():
    ...
    if escolha_jogador == escolha_goleiro and passar_goleiro == 1:
        ...
        if evento.type == pygame.MOUSEBUTTONDOWN and estado_jogo == OPCAO_ESCOLHER:
            ...
            for i, (cx, cy, raio) in enumerate(zonas_gol_circulos):
                if ponto_dentro_circulo(pos_mouse[0], pos_mouse[1], cx, cy, raio):
                    ...
                    # Modo facil -> portanto tem uma chance de 50% do goleiro deixar a bola passar
                    # 0 -> Passa
                    # 1 -> O goleiro pega
                    passar_goleiro = random.randint(0, 1)
